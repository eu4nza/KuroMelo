...
module.exports = {
    ...
    async execute(interaction, db, client) {
        try {
            ...
            const bannedUser = bannedList.find(banInfo => banInfo.user.id === userId);   
    
            if (!bannedUser) {
                return interaction.reply({ content: `${error} This user is already unbanned.`, ephemeral: true });
            }
            ...
            await interaction.reply({ content: `<:granted:1211653624875454484> Successfully unbanned <@${userId}>${reason ? ` with reason: ${reason}` : ''}.`, ephemeral: true });
        } catch (error) {
            console.error(error);
