        ...
        try {
            ...
            if (logCommandChannelID) {
                ...
                if (logCommandChannel) {
                    ...
                    .setColor('#2ecc70')
                    .setTitle('A user has been untimeout')
            ...
            await interaction.reply({ content: `<:granted:1211653624875454484> Successfully untimeout ${user}${reason ? ` with reason: ${reason}` : ''}.`, ephemeral: true });
        } catch (error) {
            console.error(error);
