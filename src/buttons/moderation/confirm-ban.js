...
module.exports = {
    ...
    async execute(interaction, db, client) {
        try {
            ...
            if (bannedUser) {
                return interaction.reply({ content: `${error} This user is already banned.`, ephemeral: true });
            }
            ...
            await interaction.reply({ content: `<:granted:1211653624875454484> Successfully banned ${user}${reason ? ` with reason: ${reason}` : ''}.`, ephemeral: true });
        } catch (error) {
            console.error(error);
